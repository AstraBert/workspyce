...
fn find_workspace_members(file_path: &Path) -> Vec<String> {
    ...
    let workspace_members = re.captures(&contents).expect(
        "No match found for members in tool.uv.workspace, are you sure you are in a uv workspace?",
    );
...
fn git_status_files() -> Vec<String> {
    let cmd = Command::new("git")
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("Error executing `git status --short`, check for git installation");
    ...
    let output_str = String::from_utf8(output.stdout)
        .expect("Could not convert standard output content to string");
    let re = Regex::new(r"[(M|T|A|D|R|C|U|\?|!)]{1,2}\s").unwrap();
    let lines: Vec<String> = output_str
        .split("\n")
        .map(|s| re.replace(s, "").trim().to_string())
        .collect();
    lines
}
...
fn find_project_name(pyproject: &Path) -> String {
    ...
    let contents = fs::read_to_string(pyproject).expect("Should have been able to read the file");
    let package_name = re.captures(&contents).expect(
        "No package name found, ensure that you follow the `[package]\nname = .*` convention.",
    );
...
fn ask_and_save_version_bump(pyproject: &Path, processed: &mut Vec<String>) {
    ...
    println!(
        "Since modifications have been detected within package `{}`, what version bump would you like to apply? [major/minor/patch/ignore]",
        project_name
    );
    ...
    if vbump.trim().to_lowercase() == "major"
        || vbump.trim().to_lowercase() == "minor"
        || vbump.trim().to_lowercase() == "patch"
    {
        println!(
            "Please provide a brief description of what changed for this {} release",
            vbump.trim().to_lowercase()
        );
        ...
        if !Path::new(".workspyce").exists()
            && let Err(why) = fs::create_dir(".workspyce")
        {
            println!(
                "Error while creating `.workspyce` directory: {:?}",
                why.kind()
            )
        }
        ...
        if let Err(why) = fs::write(&fl_name, contents) {
            println!(
                "Error while writing version changelog to `{}` directory: {:?}",
                &fl_name,
                why.kind()
            )
        }
...
pub fn check(path: &str) {
    ...
    for file in &files {
        if !file.is_empty() && is_workspace_member(&members, file) {
            println!(
                "Detected changes in {}, which is part of the workspace",
                file
            );
            match find_pyproject(Path::new(file)) {
                Ok(result) => {
                    ask_and_save_version_bump(&result.join("pyproject.toml"), &mut processed)
                }
                Err(e) => println!("{}", e),
            }
        }
...
mod tests {
    ...
    fn test_find_workspace_members() {
        assert_eq!(
            find_workspace_members(Path::new("testfiles/proj/pyproject.toml"))[0],
            "testfiles/.*"
        );
    }
    ...
    fn test_find_pyproject() {
        match find_pyproject(Path::new("testfiles/proj/childpath/anotherfile.txt")) {
            ...
            Err(e) => {
                eprintln!(
                    "Error during test when no error should have happened: {}",
                    e
                );
                assert!(false)
            }
        ...
        match find_pyproject(Path::new("testfiles/notapyproject.toml")) {
            ...
            Err(e) => {
                assert_eq!(
                    e,
                    "Could not find a parent path that contains a pyproject.toml"
                )
            }
        }
    }
    ...
    fn test_find_project_name() {
        assert_eq!(
            find_project_name(Path::new("testfiles/proj/pyproject.toml")),
            "toml-workspace"
        );
    }
    ...
    fn test_is_workspace_member() {
        ...
        assert!(is_workspace_member(
            &members,
            "packages/coding_agent/hello.py"
        ));
        assert!(!is_workspace_member(
            &members,
            "package/coding_agent/hello.py"
        ))
    }
