...
pub fn release(token: &str) -> Result<bool, String> {
    ...
    match fs::exists(Path::new(".workspyce/release.txt")) {
        Ok(true) => {
            packages = fs::read_to_string(Path::new(".workspyce/release.txt"))
                .expect("Should be able to read release.txt, since it exists.");
            ...
            for p in to_release {
                ...
                let cmd = Command::new("uv")
                    ...
                    .arg(p.trim())
                    .spawn()
                    .expect("Error executing `uv build`, check for uv installation");
            ...
            fs::remove_file(Path::new(".workspyce/release.txt"))
                .expect("should be able to remove `.workspyce/release.txt`");
            Ok(true)
        }
        Ok(false) => Err("No release.txt file, nothing to release".to_string()),
        Err(e) => Err(format!("{}", e)),
    }
}
