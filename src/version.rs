            ...
            for entry in entries {
                match entry {
                    Ok(entry) => {
                        ...
                        let ext = pt
                            .extension()
                            .expect("should be able to get an extension for the file");
                        if ext == "md" {
                            changelogs.push(pt);
                        }
                    }
...
fn bump_all_versions(changelog_paths: Vec<PathBuf>) {
    ...
    let re_version = Regex::new(r#"(?m)^version\s*=\s*"(\d+\.\d+\.\d+)"$"#)
    ...
    for path in &changelog_paths {
        ...
        let package = re_package
            .captures(&content)
            .expect("Package should be part of the changelog file");
        let pyproject = re_pyproject
            .captures(&content)
            .expect("Pyproject path should be part of the changelog file");
        let vbumb = re_vbump
            .captures(&content)
            .expect("Version bump information should be part of the chang elog file.");
        ...
        let pyp_content = fs::read_to_string(Path::new(&pyproject_path))
            .expect("Should be able to read the path to pyproject");
        let version = re_version
            .captures(&pyp_content)
            .expect("pyproject.toml file should contain a version field");
        ...
        let new_semver: String = if vbump_release.trim() == "patch" {
            let num: i32 = ver_components[2]
                .parse()
                .expect("Version components should be numbers");
            format!("{}.{}.{}", ver_components[0], ver_components[1], num + 1)
        } else if vbump_release.trim() == "minor" {
            let num: i32 = ver_components[1]
                .parse()
                .expect("Version components should be numbers");
            format!("{}.{}.0", ver_components[0], num + 1)
        } else {
            let num: i32 = ver_components[0]
                .parse()
                .expect("Version components should be numbers");
            format!("{}.0.0", num + 1)
        };
        ...
        match fs::exists(Path::new("CHANGELOG.md")) {
            Ok(true) => {
                let changelog_content = fs::read_to_string(Path::new("CHANGELOG.md")).expect(
                    "Since CHANGELOG.md exists in .workspyce/, we should be able to read it",
                );
                let new_content = format!("## {}\n\n{}\n\n", package_name, changelog_message);
                let full_content = new_content + &changelog_content;
                fs::write(Path::new("CHANGELOG.md"), full_content)
    ...
    fs::write(".workspyce/release.txt", to_release_str)
        .expect("Should be able to write release.txt");
}
